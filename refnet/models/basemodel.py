...
from ckpt_util import load_weights, delete_states
...
class CustomizedLDM(LatentDiffusion):
    ...
    def sample(
        ...
        if sampler.startswith("diffuser"):
            ...
            # Pass gram_visualizer to CFGDenoiser if available
            if hasattr(self, 'gram_visualizer') and self.gram_visualizer is not None:
                sampler.sd.gram_visualizer = self.gram_visualizer
    ...
    def retrieve_attn_modules(self):
        ...
        if self.is_sdxl:
            ...
            self.attn_modules = {
                ...
                "bottom": [6],
                "encoder": [0, 1, 2, 3, 4, 5],
                "decoder": [7, 8, 9, 10, 11, 12, 13, 14, 15]
            }
        ...
        if isinstance(scale_kwargs, dict):
            if scale_kwargs["level_control"]:
                for key in scale_kwargs["scales"]:
                    if self.is_sdxl and key == "middle":
                        continue
