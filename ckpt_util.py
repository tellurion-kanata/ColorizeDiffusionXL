"""Checkpoint utilities for loading, converting, and post-processing model weights.
Supports safetensors, PyTorch, and DeepSpeed ZeRO checkpoint formats.

Can also be invoked as a CLI tool:
    python ckpt_util.py post <checkpoint_dir>           # Extract weights from DeepSpeed checkpoint
    python ckpt_util.py post-ema <checkpoint_dir>       # Extract EMA weights
    python ckpt_util.py merge <output> <ckpt1> <ckpt2>  # Merge multiple checkpoints
    python ckpt_util.py filter <ckpt> <key_pattern>     # Remove keys matching pattern
    python ckpt_util.py convert <ckpt> <output>         # Convert SD checkpoint format
"""
...
# Prefixes of modules excluded when extracting trained weights (e.g., frozen encoders, EMA shadow).
EXCLUDE_PREFIXES = ("cond_stage_model", "model_ema", "scalar_embedder", "img_embedder", "txt_embedder")


def _should_exclude(key):
