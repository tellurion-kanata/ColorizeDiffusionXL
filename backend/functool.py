...
def preprocess_sketch(sketch, resolution, preprocess="none", extractor=None, new=False):
    ...
    if preprocess in ("none", "invert"):
        # Resize and pad as PIL first, then normalize (matches training order)
        sketch = resize((int(h * r), int(w * r)))(sketch)
        sw, sh = sketch.size
        padded = Image.new('RGB', (tw, th), (255, 255, 255))
        left = (tw - sw) // 2
        top = (th - sh) // 2
        padded.paste(sketch, (left, top))
        original_shape = (left, top, sw, sh)
        sketch = to_tensor(padded, inverse=(preprocess == "invert"))
    else:
        if preprocess == "invert-webui":
